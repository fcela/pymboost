...
def test_python_glmboost_matches_r_for_numeric_factor_star_formula(
    r_factor_star_reference_runner,
):
    x = np.linspace(-1.0, 1.0, 18)
    g = np.array(["a", "b", "c"] * 6, dtype=object)
    intercepts = {"a": -0.25, "b": 0.1, "c": 0.4}
    slopes = {"a": -0.5, "b": 0.75, "c": 1.5}
    y = np.array([intercepts[level] + slopes[level] * value for value, level in zip(x, g)], dtype=np.float64)
    ...
    r_result = r_factor_star_reference_runner(
        x,
        g,
        y,
        x_new=x_new,
